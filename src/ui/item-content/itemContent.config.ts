...
export default {
  itemContent: {
    root: cva("flex flex-1 flex-wrap items-start gap-x-2 gap-y-1"),
    content: cva("flex min-w-40 flex-1 flex-col items-start gap-4"),
    group: cva("flex w-full shrink-0 flex-col gap-1"),
    header: {
      root: cva("flex w-full shrink-0 flex-wrap items-start gap-x-4 gap-y-1"),
    ...
    label: cva("text-display font-medium whitespace-nowrap", {
      variants: {
        size: {
          sm: "text-md-tight",
          md: "text-md"
