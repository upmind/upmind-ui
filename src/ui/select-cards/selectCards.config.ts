...
export const triggerVariants = cva(
  `border-control-border h-auto min-h-10 min-w-0 items-center justify-start rounded-lg px-4 py-3 text-left font-medium`,
  {
    variants: {
      width: {
        xs: "!w-dropdown-xs",
...
export const contentVariants = cva(
  `bg-control-background ${ringClasses} ${invalidRingClasses} border-control-border mt-2 flex max-h-72 !w-[--radix-dropdown-menu-trigger-width] flex-col overflow-hidden overflow-y-scroll rounded-lg border shadow-sm`,
  {
    variants: {
      width: {
        xs: "w-dropdown-xs",
