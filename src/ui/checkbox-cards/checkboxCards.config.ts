...
export default {
  checkboxCards: {
      ...
      `text-control-foreground group flex items-start gap-2 p-3 pr-4 transition-all duration-200`,
      {
        variants: {
          isList: {
            ...
            false: `hover:border-border-border-control-hover border-border-control bg-background-control-surface rounded border ${ringClasses} ${invalidRingClasses}`
          }
        }
      }
    ),
    ...
    input: cva(
      `${groupRingClasses} border-border-control text-control-active static pr-0 leading-normal`,
      {
        variants: {
          isList: {
            true: "pl-0!",
