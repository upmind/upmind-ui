// -----------------------------------------------------------------------------
/**
 * @fileoverview client-email table channel — `ControlledTableChannel` over the
 * REAL collection (Task 36, ADR-027 Am.3)
 *
 * ## Job To Be Done
 * `useTableChannel` is the only bridge between the harness's frozen FLAT
 * `TableModel` / `TableIntent` and this module's NESTED query model, so it is
 * driven here against the real `useClientEmails().as(ScopeActorTypes.CLIENT)`
 * cell and the RECORDED 3-row corpus: `read()` flattens the live
 * `useContext().query` + `.pagination` DOWN to the renderer, and every
 * `TableIntent` lifts UP into the composable's own `filterBy` / `sortBy` /
 * `nextPage` / `prevPage`, the channel owning no state of its own. The channel
 * lives in `playgrounds/labs-nuxt` because `packages/headless` has no
 * `@upmind-automation/scenario-harness` dependency; Task 36's Reality Check
...
import { useTableChannel } from "../../../../../../playgrounds/labs-nuxt/app/composables/factory/useTableChannel";
