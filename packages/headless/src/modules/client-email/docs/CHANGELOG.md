...
- **A real, drivable rendering of the filter bar.** `@upmind-automation/client-vue`'s `Filter` renderer resolves this module's query schema alone — a tri-state switch for each boolean status column, a search box for `email`, chosen purely from the column's own declared operators — and renders live end to end in the `labs-nuxt` playground. See this module's [README](./README.md#playground) for the exact command and url.
