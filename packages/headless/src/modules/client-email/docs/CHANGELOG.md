...
- **A separate uischema for the sort control's own option labels** — `useContext().schemas.query.sortUischema`, one `Control` over the `sort` branch whose `i18n` key doubles as the option-key prefix (`<i18n>.<field>`), the same tri-state prefix mechanism the filter controls above use. The `sort` branch's own schema stays a bare `enum` with no `title` and no `i18n`; a sort option's label always resolves through this uischema, never the schema. See [usage.md](./usage.md#sort-ui-schema).
