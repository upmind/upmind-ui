 ...
 *
 * ## Job To Be Done
 * Capture the real `clients/{id}/companies` responses this module's criteria
 * migration emits, so its integration specs replay recorded reality:
 *
 ...
 * `filter[name|like]` wire form answered by the API.
 *
...
describe("Client-Company API Fixtures Generator", () => {
  ...
  beforeAll(async () => {
    ...
    const probe = await fetch(
      `${API_URL}/api/clients/${clientId}/companies?limit=2`,
      {
        headers: {
          Accept: "application/json",
          Origin: ORIGIN,
    ...
    if (!needle) {
      throw new Error(
        "The staging client holds no named company — the filter capture has " +
  ...
  it("captures the limit=2 walk (pages 1 and 2)", async () => {
    ...
    const pageOne = await generator.get(
      `/api/clients/${clientId}/companies?limit=2&offset=0&case=page-1`
    );
    const pageTwo = await generator.get(
      `/api/clients/${clientId}/companies?limit=2&offset=2&case=page-2`
    );
  ...
  it("captures filter[name|like] — the migrated wire form", async () => {
    ...
    const { status, body } = await generator.get(
      `/api/clients/${clientId}/companies?filter[name|like]=${encodeURIComponent(
        `%${needle}%`
