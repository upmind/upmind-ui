...
This is a new unified address flow, where we dont differentitate between a normal address and a company. We have a single address flow that can be used for both.
...
- we dont use objects for company addresses, phones and emails, which are combersome to add
...
- everythin gis added asynchrounously, so that we can add new values to the lookup values as they are created, we just ensure uniqueness
