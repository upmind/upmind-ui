...
export function parseDomain(raw: any, _force = false): DomainModel | undefined {
  const domain = (isObject(raw) ? get(raw, "domain") : raw) ?? "";
...
export function parseSuggestions(
  ...
  const { defaultPaymentPeriod: _defaultPaymentPeriod } = useBrand();
  const available = map(results, result => {
    const { sld, can_register, can_transfer, product_id } = result;
    const tld = trimStart(result.tld, ".");
