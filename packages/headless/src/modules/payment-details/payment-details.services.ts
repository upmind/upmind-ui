...
export function loadList() {
  ...
  return query<IPaymentDetail[], PaymentDetail[]>({
    ...
    enabled: () => isAuthenticated.value && !!clientId.value && !!brandId.value
  });
}
...
async function loadLookups(
  ...
  const { isAuthenticated } = useActiveSession().useMeta();
  const _paymentTypes: Record<string, PaymentType> = {
    ["PAY_IN_FULL"]: PaymentType.PAY_IN_FULL
  };
  if (!isAuthenticated.value || !client?.id) throw new NotAuthenticatedError();

  const { brandId, currencyId: defaultCurrencyId, ensureConfig } = useBrand();
  const { get: getRequest, post: _post, useUrl } = useQuery();
