...
// --- internal
import paymentDetailMachine from "./paymentDetail.machine";
...
// --- types
import type { PaymentDetailsAddArgs } from "./types";
...
export const usePaymentDetailAdd = ({
  ...
  // Most gateways need a currency, so if we are not provided then lets determine from the client or brand
  const safeCurrency =
    currency ?? first(client.value?.accounts)?.currency ?? brandCurrency.value;
