...
import { basketProductServices } from "../basket-product";
...
import { basketProductRequiresSetup } from "./product-setup.utils";
import {
  compactDeep,
  contextValue,
  isDeepEmpty,
...
export function useProductSetup() {
  ...
  function captureSchemas(): void {
    ...
    const _basketErrors = contextValue<ProductConfigContext["basketErrors"]>(
      currentConfig.value?.service,
      "basketErrors"
    );
