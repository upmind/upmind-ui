...
> Sample trimmed for readability — additional categories ("Domain Names", "Web Hosting", "Design Services", "Frankfurt, Germany", "Managed Hosting for Wordpress", "Variants") and the full per-node administrative fields (timestamps, `import_id`, `staged_import`, `ui_settings`, `translations`) are preserved in the captured fixture at [`tests/fixtures/recordings/get-basket-products_categories-f13c8b36.json`](../../../../../../tests/fixtures/recordings/get-basket-products_categories-f13c8b36.json). The `meta` field present on every node in the capture has been stripped per the top-of-doc note.
