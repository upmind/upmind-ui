...
function createClientAddressesForScope(
  ...
  // Takes no params: the request state is the declared query schema
  // (`client-address.schemas.ts`'s `useQuerySchema()`), handed to
  // `list({ criteria })` inside the services layer.
