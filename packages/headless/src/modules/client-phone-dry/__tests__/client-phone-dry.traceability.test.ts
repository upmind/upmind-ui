/**
 * @fileoverview client-phone-dry — feature <-> test traceability (unit)
 *
 * ## Job To Be Done
 * `client-phone-dry.feature` is the module's non-executable behavioural source
 * of truth. This test keeps it honest against the real suite, both ways:
 *   - every scenario (except ones tagged @todo) is proven by at least one test
 *     that names its @AC-* id — a scenario with no test is a coverage hole;
 *   - every AC-* id a test names exists as a scenario — a test tethered to a
 *     scenario that isn't there is untethered / stale.
 * It is the anchor the factory promises ("a scenario with no proving test is a
 * visible coverage hole, a behavioural test with no scenario is untethered",
 * `.claude/skills/scoped-composable-factory/SKILL.md`). It is a test, not a CI
 * gate — it rides the module's own suite.
 *
 * ## Layer
...
const FEATURE = join(HERE, "client-phone-dry.feature");

// Any AC id token: AC-A1, AC-12a, AC-S1, AC-B4, AC-MATRIX, AC-CART…
...
const testFiles = readdirSync(HERE).filter(
  f => f.endsWith(".test.ts") && f !== "client-phone-dry.traceability.test.ts"
);
...
describe("client-phone-dry.feature <-> tests traceability", () => {
  ...
  it("every AC id named by a test exists as a scenario (no untethered test)", () => {
    ...
    expect(
      untethered,
      `tests naming an AC id that has no scenario in client-phone-dry.feature: ${untethered.join(", ")}`
    ).toEqual([]);
  });
});
