...
import { getBasketProduct } from "./basket-product.utils";
import {
  contextValue,
  DEBOUNCE_DELAY,
  DetailedError,
...
export const useBasketProduct = (
  ...
  const rawBasketProduct = getBasketProduct(bpid, rawBasket.value);
  ...
  const _updateQuantity = debounce(
  ...
  const _incrementQuantity = debounce(
    async (): Promise<void> =>
      getProduct().then(product => {
        if (!product?.productDetails.quantifiable)
          return Promise.reject(
  ...
  const _decrementQuantity = debounce(
    async (): Promise<void> =>
      getProduct().then(product => {
        if (!product?.productDetails.quantifiable)
          return Promise.reject(
