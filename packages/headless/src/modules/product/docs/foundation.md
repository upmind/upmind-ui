...
type BlueprintField = {
  ...
  description: string | null;
  type: string; // "text" | "select" | "checkbox" | "number" | …
  required: boolean;
  order: number;
  group: string | null; // optional UI grouping
  options?: Array<{ value: string; label: string }>; // populated for select-style fields
  validation?: Record<string, unknown>; // regex / min / max / typed constraints
  default?: string | number | boolean | null;
  category?: { id: string; name: string }; // populated by the `category` expand
};
```
