...
The module has **no mutation surface at all** — no schema layer, no state machine, no `mutate()`. Both composables exist purely to read.
...
`useContext()`, `useMeta()` and `useInternals()` are lazy — they build on call. `useActions()` is **not**: it is minted once per scope, because the applied `filters` ref lives inside it. A factory minted per call would give every handle its own filter state.
...
| ---------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `useActions()`   | 8 members — sort, filter, page, lifecycle                                                             | 4 members — lifecycle only                                                         |
| `useContext()`   | 5 members — reactive list, lookups, captured error, pagination                                        | 2 members — the mapped email, captured error                                       |
...
One services file (`client-email-history.services.ts`) serves both halves. There are no per-actor service variants today — the only resolving actor in either scope matrix is `client` — and the same holds across every other layer (actions, context, meta; there is no schema layer at all, because there is no mutation surface to validate).
...
Everything else is internal and carries a file-level internal marker: the services and the mappers. The services file in particular must never be imported directly from outside this module — both composables resolve it internally.
