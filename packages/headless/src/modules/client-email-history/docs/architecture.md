...
| Query schema (collection only) | `useQuerySchema()` / `useQueryUischema()` / `useSortUischema()` (`client-email-history.schemas.ts`) — declares what `setCriteria` accepts, what a filter-bar renders, and (a separate third piece) what a sort control's own option labels resolve through; all three published at `useContext().schemas.query`, consumed only by the collection's `loadList` (`list({ criteria: { schema } })`) |
...
Everything else is internal and carries a file-level internal marker: the services, the mappers, and the query schema factories (`useQuerySchema()` / `useQueryUischema()` / `useSortUischema()`). The services file in particular must never be imported directly from outside this module — both composables resolve it internally. A consumer's only door to the schema trio is `useContext().schemas.query`, never a direct import of the schema factories.
