...
import type { PhoneContext, PhoneModel } from "./client-phone.types";
import type { ResponseError, UseActor } from "../../utils";
import type { ScopeActorTypes } from "../scope/scope.types";
import type { ErrorObject } from "ajv";
// -----------------------------------------------------------------------------
/**
 * @module client-phone/useClientPhoneManager.context
 ...
 * `useClientPhoneManager.machine.ts`'s `setSchemas`, live in machine context,
 * and reach consumers HERE — the barrel exports no bare pair (decision D-4),
 * because a form rendered from a schema the machine has not adopted validates
 ...
 */
export function createClientPhoneManagerContext(
  ...
  // earns one, add `useClientPhoneManager.context.{actor}.ts` and spread it
  // LAST.

  return {
    /** The full data-manager context object. */
    context: useContext<PhoneContext>(state),

    /** Description of the phone being managed — the resolved country name. */
    ...
    /** The id of the phone being managed (undefined for a new draft). */
    id: useContext<string | undefined>(state, "id"),

    /** The current form model. */
    model: useContext<PhoneModel | undefined>(state, "model"),

    /** The JSON schema for the form (from machine context — see JSDoc). */
    schema: useContext<PhoneContext["schema"]>(state, "schema"),

    /** Display title of the phone — the model's `phone.number`. */
    title: useContext<string | undefined>(state, "title"),

    /** The UI schema for the form (from machine context — see JSDoc). */
    uischema: useContext<PhoneContext["uischema"]>(state, "uischema"),
...
export type UseClientPhoneManagerContext = ReturnType<
  typeof createClientPhoneManagerContext
>;
