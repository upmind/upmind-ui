    ...
    const createTranslator =
      (_locale: string) => (key: string, defaultMessage: string, data: any) => {
        ...
        if (key.startsWith("validation.")) {
          // console.debug("Translating validation key:", key, data);
          ...
          if (validationKey === "format" && data?.format) {
            ...
            const formatSpecificError = isFunction(t)
              ? t(formatSpecificKey, { ...data })
              : undefined;
            // If we found a format-specific message, use it
            if (
              formatSpecificError &&
              formatSpecificError !== formatSpecificKey
            ) {
              return formatSpecificError;
            }
          }

          // If this is not an additional error, ensure the data object has a property matching the validation key
          ...
          if (!isAdditionalError && !isEmpty(data?.i18n) && isFunction(t)) {
