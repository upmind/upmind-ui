...
export default {
  ...
  product: {
    ...
    card: {
      ...
      footer: cva(
        "flex max-w-xs flex-1 items-center justify-start gap-6 text-right"
      ),
      // ---
      loading: cva("size-7"),
      title: cva(
    ...
    config: {
      ...
      content: cva("flex flex-wrap items-start"),
      ...
      grid: {
        ...
        items: cva(
          "m-0 grid w-full list-none grid-cols-[repeat(auto-fit,minmax(16rem,1fr))] gap-3 p-0"
        ),
        item: {
          root: cva(
