import { cva } from "class-variance-authority";
// -----------------------------------------------------------------------------
const defaultVariants = {
  anchor: "none",
  variant: "stacked",
  color: "base",
};
// -----------------------------------------------------------------------------
export default {
  alert: {
    root: cva("inline-flex gap-2  p-4 px-2 py-2", {
      variants: {
        anchor: {
          none: "rounded-lg",
          left: "rounded-r-lg border-l-4 border-current",
          right: "rounded-l-lg border-r-4 border-current",
          top: "rounded-b-lg border-t-4 border-current",
          bottom: "rounded-t-lg border-b-4 border-current",
        },
        variant: {
          stacked: "flex-wrap items-start",
          inline: "flex-nowrap items-center",
        },
        color: {
          base: "border-base bg-base-50 text-base-900",
          current: "border-current bg-base-100 text-current",
          primary: "border-primary bg-primary-50 text-primary-900",
          secondary: "border-secondary bg-secondary-50 text-secondary-900",
          accent: "border-accent bg-accent-50 text-accent-900",
          success: "border-success bg-success-50 text-success-900",
          error: "border-error bg-error-50 text-error-900",
          warning: "border-warning bg-warning-50 text-warning-900",
          info: "border-info bg-info-50 text-info-900",
        },

        block: {
          true: "flex w-full",
          false: "w-full max-w-sm",
        },
      },

      defaultVariants,
    }),

    icon: cva("size-6 p-1", {
      variants: {
        color: {
          base: "text-base",
          current: "text-current",
          primary: "text-primary",
          secondary: "text-secondary",
          accent: "text-accent",
          success: "text-success",
          error: "text-error",
          warning: "text-warning",
          info: "text-info",
        },
      },
      defaultVariants,
    }),

    content: cva("flex w-full flex-1 overflow-hidden", {
      variants: {
        variant: {
          stacked: "flex-col items-start pb-2",
          inline: "flex-row items-center",
        },
      },
      defaultVariants,
    }),

    title: cva("m-0 font-medium leading-6 text-inherit", {
      variants: {
        variant: {
          stacked: "wrap",
          inline: "wrap flex-grow-1 flex-shrink-0",
        },
        color: {
          base: "text-base",
          current: "text-current",
          primary: "text-primary",
          secondary: "text-secondary",
          accent: "text-accent",
          success: "text-success",
          error: "text-error",
          warning: "text-warning",
          info: "text-info",
        },
      },
      defaultVariants,
    }),

    text: cva("m-0 text-sm", {
      variants: {
        variant: {
          stacked: "",
          inline: "flex-1 truncate leading-6  ",
        },
      },
      defaultVariants,
    }),

    data: cva("m-0 text-xs", {
      variants: {
        variant: {
          stacked: "",
          inline: "truncate leading-6",
        },
      },
      defaultVariants,
    }),

    actions: cva("", {
      variants: {
        variant: {
          stacked: "self-start",
          inline: "self-center",
        },
      },
      defaultVariants,
    }),

    close: cva("!size-4 !rounded-full !p-3 [&>*>.icon]:!size-4 "),
  },
};
