...
export default {
  guardSession: async ({
    ...
    try {
      const isAuth =
        session.meta.value.isAuthenticated || (await session.isAuthenticated());
    ...
    // Avoid redirecting to the same route we are already on
    if (
      resolvedRoute.path === currentRoute.path ||
      resolvedRoute.name === currentRoute.name
    ) {
      return {
        type: FunnelActions.NEXT
      };
