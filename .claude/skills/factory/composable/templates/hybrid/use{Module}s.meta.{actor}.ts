 ...
 * exclusive to the `client` actor, or overrides `useModules.meta.ts`'s shared
 * implementation — never as an empty scaffold (clause 2,
 * `code-composables.companion.md` "Variance law"). Otherwise DELETE this
 * file; the armless shared factory in `useModules.meta.ts` suffices. See
 ...
 * conceptual pair as `templates/machine/useModules.meta.{actor}.ts`'s own
...
// -----------------------------------------------------------------------------
/**
 * @module module/useModules.meta.client
...
export function createClientModuleMeta(query: ModuleListQuery) {
   ...
   * `useModules.actions.{actor}.ts`'s own `registerAsGuest`).
   *
   * @single-source If this capability flag ALSO gates an `actions`-arm member,
   * do NOT recompute it here — compute it once (e.g. in `useModules.ts` or a
   ...
   * `useModules.meta.ts`'s shared factory; this arm's version is spread LAST
