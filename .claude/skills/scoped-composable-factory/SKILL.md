...
6. **Worktree** — where the run executes (opened by this run itself at conduction start — `/start` is the full-lifecycle door, not a worktree opener, and is never invoked mid-factory since it would dispatch a second orchestration of the same story; keep = `/pause`, clean = `/complete`, list = `/resume` — the former `/worktree` door is folded into these).
...
| Code | `/code` (routes to the scoped-composable scaffold + generate crafts) with this skill's own `templates/{machine,query}/` set | developer | **diff file count** > 0 AND **hand-off-filed** = true (public-surface contract handed to Tests; diff/hand-off withheld from it — ADR-029) |
...
| Verify | `/review` (verify lane) | verifier | **verdict field** = `PRESENT` (ABSENT halts — see Failure states) |
| Review | `/review` (code lane; loads this repo's `code-reviews.companion.md`, which carries the FE-2967 variance-law cues) | reviewer (pre-gate) | **🔴 blocker count** = 0 |
...
- Code/Tests/Review/Verify/Docs (developer → prover → reviewer → verifier → documenter) → `${CLAUDE_PLUGIN_ROOT}/teams/dev-story.yaml`. The Review stage's reviewer-model detail (mixed opus/sonnet/haiku finder panel) is `${CLAUDE_PLUGIN_ROOT}/teams/review-fanout.yaml`, invoked by `/review` itself — cited, not restated here.
