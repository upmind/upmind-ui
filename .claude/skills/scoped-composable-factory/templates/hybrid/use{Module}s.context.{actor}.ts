 ...
 * exclusive to the `client` actor, or overrides `useModules.context.ts`'s
 ...
 * file; the armless shared factory in `useModules.context.ts` suffices. See
 ...
 * `useModules.actions.{actor}.ts` (this template set's own actions arm), cited
 ...
 * `templates/machine/useModules.context.{actor}.ts`'s own (there: `entitlements` /
...
import { baseLookups } from "./useModules.context";
import type { ModuleItem, ModuleListQuery } from "./module.types";
// -----------------------------------------------------------------------------
/**
 * @module module/useModules.context.client
...
export function createClientModuleContext(query: ModuleListQuery) {
   ...
   * `useModules.context.ts`'s shared factory; this arm's version is spread
