 ...
 * in `module.schemas.ts` suffice. See `.claude/skills/scoped-composable-factory/templates/ARMS.md` for the full decision tree.
 *
 ...
 *
 * QUERY <-> MACHINE SYMMETRY — this file's two worked members
 * (`useClientRegisterSchema` exclusive,
 * `useSchema` overriding) are the SAME conceptual pair as the
 ...
 * `module.services.{actor}.ts` / `useModules.actions.{actor}.ts`'s own
 * `registerAsGuest`.
 */
...
/**
 * OVERRIDING MEMBER worked example — same key (`useSchema`) as
 ...
 * @decision
 * what: this arm's `useUischema` renders the shared controls PLUS controls for
 *   the fields its own schema override introduced (`id`, `phone`), and
 *   re-declares `name` with different options.
