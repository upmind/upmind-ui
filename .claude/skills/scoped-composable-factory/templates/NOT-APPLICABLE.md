...
| A second public composable | both | Not present | This template set covers ONE scoped composable. A module needing a second (auth's `useVerifyEmail.ts` with its own `auth.services.client.email.ts`; client-email's `useClientEmailManager`, a per-item form on the shared `dataManagerMachine`) repeats this same template set — or the other variant, for a form beside a collection — under its own `use{Second}.ts` name. **Backed by a SHARED machine (`dataManagerMachine`) instead of its own `createMachine`? Then it carries NO `{module}.machine.ts` — it configures the shared machine via the `use{Module}.machine.ts` config-factory template (`create{Second}MachineConfig(service): Parameters<typeof dataManagerMachine.withConfig>[0]`), per `code-scoped-composable.companion.md` "Shared-machine config factory". No on-disk module is the exemplar — every current consumer is a migration target still on the old inline `as any ×3` shape.** |
