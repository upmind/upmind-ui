...
2. **Read provision-field definitions separately.** Issue `GET /basket/products/{id}/provision_fields?sub_product_ids=<selected option/attribute ids, csv>&lang=<active>`. Response is a `BlueprintField[]` (see `product.md` "Provision-field definitions"). On initial product-page load `sub_product_ids` is empty (no selections yet); the call returns the fields that apply with no sub-products selected.
...
type ProvisionField = {
  ...
  id: string;
  code: string;                   // stable machine identifier (e.g. "domain")
  label: string;
  description: string | null;
  type: string;                   // "text" | "select" | "checkbox" | "number" | … — see product.md
  required: boolean;
  order: number;
  group: string | null;
  options?: { value: string; label: string }[]; // for select-style types
  validation?: Record<string, unknown>;          // regex / min / max / typed constraints
  default?: string | number | boolean | null;
  category?: { id: string; name: string };       // populated when the `category` expand is requested
};
...
| 2a | GET | `/basket/products/{productId}/provision_fields?sub_product_ids=&lang=<lang>` | Initial provision-field definitions on product-page load (no sub-products selected yet). Returns `BlueprintField[]` | _(see product module's fixture index for `get-basket-products-{productId}-provision_fields-*.json`)_ |
| 2b | GET | `/basket/products/{productId}/provision_fields?sub_product_ids=<csv>&lang=<lang>` | Re-read after the user picks / unpicks an option or attribute — different selections can unlock or hide different fields | Same shape as 2a; different query string |
