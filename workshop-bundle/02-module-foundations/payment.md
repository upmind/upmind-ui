...
```ts
// The body for POST /payments. Confirmed against the captured v2 fixture
// `tests/__fixtures__/recordings/post-payments.json` (new-card Stripe inline path).
type PaymentPost = {
  invoice_id: string;          // the invoice being charged
  gateway_id: string;          // chosen gateway (from the eligibility list)
  client_id: string;           // the paying client — required on the body, not derived from the bearer
  amount: number;              // amount to charge in the invoice's currency major units (e.g. 198 = $198.00)
  return_url: string;          // post-challenge return URL. The new-card Stripe inline path nests
                               // success / failed callbacks as a query string:
                               // `?success=<urlencoded>&failed=<urlencoded>`.
  cancel_url: string;          // URL the user lands on when the gateway hand-off is cancelled
  store_on_payment: boolean;           // capture-and-store: also save the card as a reusable method
  store_on_payment_auto_payment: boolean; // if storing, flag the saved method as auto-payment-eligible

  // Stored-card branch — pay with an existing method.
  payment_details_id?: string;

  // New-card branch — provider-specific payload, nested (NOT flat top-level keys).
  // For Stripe inline (PaymentIntents / Stripe.js): the SDK's `createPaymentMethod`
  // result rides under `payment_method_addition`.
  payment_method_addition?: {
    payment_method_id: string; // e.g. "pm_1TYpjPBiaNfu64y0ZF51Y8Hr" from stripe.createPaymentMethod()
    payment_method_type: string; // e.g. "card" — provider-defined sub-type
  };
};
```

Exactly one of `payment_details_id` (stored card) or `payment_method_addition` (new card) is populated per attempt; the other is omitted. Other gateway providers may key their one-off payload differently — Stripe inline uses the nested `payment_method_addition` shape above; older direct-card gateways may carry flat top-level fields. Read the gateway's `display_fields` on the brand-gateway row to confirm, or capture a representative request against the gateway in question.
...
**Stored-card branch** — reference an existing method:
...
**New-card branch (Stripe inline)** — captured shape from a real run (see `tests/__fixtures__/recordings/post-payments.json`):
...
curl -s "$API/payments?lang=en-US" \
  ...
  -H "Content-Type: application/json" \
  --data '{
    "invoice_id": "d7382485-0793-1528-694f-81e642d59e06",
    "gateway_id": "4d036794-24d0-e710-9d5a-3153698d582e",
    "client_id": "8d632507-9806-5d1e-48dc-8174e234e98d",
    "amount": 198,
    "payment_method_addition": {
      "payment_method_id": "pm_1TYpjPBiaNfu64y0ZF51Y8Hr",
      "payment_method_type": "card"
    },
    "return_url": "?success=http%3A%2F%2Fcollabstudio.localhost%3A5175%2Forder%2Fd7382485-...%3Fpayment_success%3Dtrue&failed=http%3A%2F%2Fcollabstudio.localhost%3A5175%2Forder%2Fd7382485-...%3Fpayment_success%3Dfalse",
    "cancel_url": "http://collabstudio.localhost:5175/order/d7382485-...",
    "store_on_payment": false,
    "store_on_payment_auto_payment": false
  }'
```
