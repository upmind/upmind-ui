...
## Amendment — sort labels are the module's uischema; one translator, consumed as a library (2026-08-18)

1. **§1's first bullet is narrowed.** *"Filters and sort are the composable's query schema, not the declaration's … each carrying its own field titles"* stands in full as to WHERE the vocabulary comes from, and is superseded as to where the LABEL comes from: a sort field's label is not a schema title. It is the module's own **sort uischema** `i18n` prefix ([ADR 032](032-schema-family-for-scoped-collections.md), Amendment 2026-08-18), resolved as `<i18n>.<field>`. A scenario declaration still names no filter, no sort field and no sort label — the single-source-of-truth law is unchanged; only the source moved from the schema's `title` to the uischema's `i18n`, which is where every other surface in this contract already carries it. The reciprocal ban stands and is tightened: a query schema's `title` is plain English for a human reading the schema, never an i18n key.
2. **One translator, consumed as a library — zero new JSONForms mounts.** Every declaration-sourced string a runtime surface draws — column headers, card labels, picker options, badge labels, action labels, action feedback, detail field labels, sort options — resolves through the SAME engine translator the mounted filter bar uses (`useFormI18n`), consumed as a plain function beside the two core facilities the runtime already consumes as a library (rules evaluation and cell dispatch). No surface mounts `<JsonForms>` to obtain a label: no row schema exists, action rules scope the ROW rather than a form model, actions are commands rather than data controls (ADR-027 port ownership is untouched), and `UPDATE_CORE`'s full-validate cost class does not exist today. A component's OWN copy keys stay on plain `t()` — they carry no uischema override channel to consume.
3. **Runtime surfaces translate in render-time computeds and are NOT remounted on a locale change.** `Form.vue`'s `:key="locale"` remount exists only because the engine mutates translated strings into `options` at mount; a runtime surface re-reads the translator reactively instead, so a locale switch re-labels in place and the table keeps its rows, scroll, selection and focus. A deliberate divergence — not to be "aligned".

Built and verified on `client-email` and `client-email-history`: every runtime surface named in clause 2 draws its declaration-sourced labels through the module's own sort uischema and the shared engine translator, with no new `<JsonForms>` mount.
