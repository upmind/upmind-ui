...
## Amendments — FE-2976 (2026-07-31)

Operator rulings recorded in the FE-2976 review-notes (planning workspace; mirrored as a comment on Linear [FE-2976](https://linear.app/upmind-automation/issue/FE-2976)). These amend the Decision text above; the review-panel amendments above stand except where item 2 below narrows decision 10 further.

1. **Decision 2 (siting) + §Context fact 1 — corrected.** The core lands in its own new workspace package, `packages/scenario-harness`, not "with the soon-agnostic headless core": ADR-028 (`docs/adr/028-headless-vanilla-core-split.md`, Superseded 2026-07-20) keeps headless Vue-coupled, reversing the premise decision 2 sited on. The framework-agnostic requirement itself survives — on a replaced rationale, reuse elsewhere, not the headless split.
2. **Decision 10 + the ADR-day-one "typed Scenario schema" output — superseded.** No scenario module ships. The shared spec layer is the standard BDD pair: the human `.feature` (unchanged, spec-only per ADR-020) plus a colocated `<module>.steps.ts`, both speaking only through a shared `world` interface; `playwright-bdd` runs the `.feature` natively (ADR-020 Amendment 4). What survives of decision 10 by design: the string-key indirection (`ComposableKey`), the never-DOM assertion guard (`expectMeta`), and the executor-sited registry (item 3 below). Drift defence is a bidirectional feature↔steps traceability check plus the runtime coverage gate, backstopped post-merge by a corpus-style refresh check landing with FE-2968's first feature/steps pair.
3. **Amendment 5 / decision 10 registry siting — refinement, not reversal.** The shared key manifest (`COMPOSABLE_KEY` as-const union) lives in the new core package; each executor's live-factory map stays executor-side, bound via `satisfies`. Both clauses hold simultaneously — recorded here so neither reads as contradicted.
