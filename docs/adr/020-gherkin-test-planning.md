...
### Amendment 5 — the package-colocated `.feature` is the executed artefact, not merely spec-only in intent (August 2026)

> **Supersedes:** §Decision item 2, «`.feature` files are spec-only, not executable», for the package-source-colocated home Amendment 4 created — and narrows Amendment 4's own «spec-only in intent» framing of that same home.
> **Home:** the generated-playground work that plays a module's `.feature` back as a replayable demonstration.

Amendment 4 kept the package-colocated `.feature` "spec-only in intent," reasoning that the paired `steps.ts` is the true executable surface and the `.feature` merely documents it. That framing does not survive a consumer that plays the `.feature` back: a generated page's transport bar treats the feature file as its own playlist, deriving each entry directly from the scenarios written there, and a step catalogue's coverage over that same file is what decides — scenario by scenario — which of those entries are actually driveable today. There is no second, "real" executable artefact standing behind the `.feature` in that reading; the feature text **is** the thing played, and the catalogue is what proves how much of it can be.

Consequences:

1. **One `.feature` per module holds every scenario the module owns — driveable and not-yet-driveable alike.** A scenario whose steps are not yet all implemented is not an exception, an error, or a second category of file: it simply is not yet driveable, and stays written down as spec until a step definition exists for each of its steps. No "non-executable" marking, and no second file, distinguishes the two — whether a scenario's own steps are all matched is the only fact that does.
2. **A consumer that plays the file back derives its playlist from exactly the scenarios whose steps are all matched**, so a scenario appears on that playlist the moment, and only the moment, its steps are fully implemented — never before, and never by a separate flag someone has to remember to flip.
3. **Traceability over that file is two-directional and reports what it finds, rather than merely gating on it.** A step definition matched by no scenario is dead; a scenario matched only in part is the dangerous case, because it reads as playable and silently is not; a scenario matched by nothing at all is simply not yet driveable, which is a legitimate, unflagged state; and the count of driveable scenarios against the file's total is reported on every run, so growth in the spec that outpaces the steps that drive it is a number visible to whoever is watching, not a silent gap.
4. **A capability spec kept anywhere temporary and copied into the package for this purpose is retired**, with nothing replacing it: the package-colocated `.feature` is the only file this lane, or anything built over it, treats as truth.

Item 4's declarative-style convention, Amendment 4's `playwright-bdd` execution path for this same home, and every other amendment above are otherwise unchanged.
