...
Currently, the funnel only reacts to route changes. Business state changes that require navigation (e.g., session expiry, basket becoming empty) are handled by imperative `router.push()` calls scattered throughout `App.vue` or other components. This leads to:
- Duplication of navigation logic.
- Race conditions between route resolution and state-driven navigation.
- Difficulty in testing navigation policies.
...
**What we could steal:** Funnel watcher subscriptions (FE-2546). The funnel could subscribe to specific business state changes and trigger its resolution pipeline when those states change.

```typescript
// Funnel config
watchers: {
  sessionExpired: {
    condition: (context) => !context.session.isValid,
    target: ROUTE.LOGIN
  },
  basketEmpty: {
    condition: (context) => context.basket.items.length === 0,
    target: ROUTE.CATALOGUE
  }
}
```

This would centralize state-driven navigation, prevent race conditions, and make navigation policies testable.
