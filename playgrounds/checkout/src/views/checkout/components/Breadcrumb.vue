<template>
  <nav
    class="sticky top-0 -mx-4 -mt-8 flex flex-row items-center justify-start gap-8 border-b border-base-300 bg-base px-4 text-base-content sm:-mx-6 sm:px-6 lg:-mx-20 lg:px-20"
  >
    <router-link
...
export default defineComponent({
  ...
  setup() {
    return {
      ...
      steps: computed(() => {
        return [
          { label: "Overview", hash: "#overview", active: true },
          { label: "Account", hash: "#account" },
          { label: "Payment", hash: "#payment" },
          { label: "Confirmation", hash: "#confirmation" },
        ];
      }),
      doCheckout: async () => {
        checkout();
