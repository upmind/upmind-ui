...
- **Presentation** — the part of a declaration describing how a record draws: an icon, a table, a card, and a set of actions. A table and a card are the *same* declared fields, laid out two different ways.
...
- **Scope bar** — the app-wide control cluster (brand, session, acting‑for) that decides *who* a page is looking as and *for* whom, independent of which page is open.
