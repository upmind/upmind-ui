// -----------------------------------------------------------------------------
/**
 * @fileoverview client-phone declaration — the binding claim the scenario makes
 *
 * ## Job To Be Done
 ...
 *
 * Negative controls: `useClientPhones/client-phone.must-fail.patch`.
 */

import { existsSync, readFileSync, readdirSync } from "node:fs";
import { join } from "node:path";
import { describe, expect, it } from "vitest";
import declaration from "../useClientPhones/client-phone.scenario";
import { every, filter, flatMap, includes, map, some } from "lodash-es";
...
describe("the declaration tracks a module with a committed feature", () => {
  it("names a module under packages/headless/src/modules", () => {
    expect(declaration.tracks).toBe("client-phone");
  ...
  it("the feature file tags at least one scenario", () => {
    const featurePath = join(MODULE_ROOT, "__tests__/client-phone.feature");
...
describe("the declaration's presentation covers the expected elements", () => {
  it("table declares the title text column", () => {
    const scopes = map(declaration.presentation.table?.elements, "scope");
    expect(scopes).toContain("#/properties/title");
  });

  it("table declares NO badges column — phones carry no verified capability (operator ruling)", () => {
    const badgeCells = filter(
      declaration.presentation.table?.elements,
      cell => cell.type === "TableCellBadges"
    );
    expect(badgeCells).toHaveLength(0);
  });
